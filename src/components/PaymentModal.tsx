...
interface PaymentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAccountChange?: (account: PublicAccount) => void;
}
  ...
  return (
    <>
          ...
          <div className="relative w-full max-w-md bg-white rounded-3xl p-6 sm:p-8 border-2 border-rose-200 text-gray-900 shadow-2xl z-10">
            ...
            {alreadyLifetime ? (
              ...
              <>
                ...
                {account?.type === 'guest' && (
                  <div className="text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4">
                    <p className="mb-2">
                      You&apos;re using a quick account, so your pass will only work in this browser.
                      Sign in with Google to keep it on any device. Anything you already bought moves with you.
                    </p>
                    <GoogleSignInButton callbackUrl={typeof window !== 'undefined' ? window.location.pathname : '/'} />
                  </div>
                )}
      ...
      <LoginModal
        ...
        onClose={() => {
          setShowLogin(false);
          if (!account) onClose(); // closed without logging in → close everything
        }}
        callbackUrl={typeof window !== 'undefined' ? window.location.pathname : '/'}
        onLoggedIn={(acc) => {
          setAccount(acc);
          onAccountChange?.(acc);
          setShowLogin(false); // now show the plans so they can choose
        }}
      />
    </>
  );
