...
export const InteractivePlayground: React.FC = () => {
  const [activeType, setActiveType] = useState<'would_you_rather' | 'truth_or_dare' | 'this_or_that'>('would_you_rather');
  const [currentIndex, setCurrentIndex] = useState(0);
  const [selectedOption, setSelectedOption] = useState<'A' | 'B' | null>(null);
  const [revealedTruthOrDare, setRevealedTruthOrDare] = useState<'truth' | 'dare' | null>(null);

  // Filter questions by type
  const currentQuestions = HERO_TEASER_QUESTIONS.filter(q => q.type === activeType);
  ...
  return (
    <section id="featured-playground" className="py-8 px-4 sm:px-6 max-w-5xl mx-auto scroll-mt-20">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <button
                ...
                className={`relative group text-left p-6 rounded-2xl border-2 transition-all duration-200 flex flex-col justify-between min-h-[140px] ${
                  selectedOption === 'A'
