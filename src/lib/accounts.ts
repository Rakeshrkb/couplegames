...
export interface Account {
    ...
    createdAt: number;
    lastSeenAt: number;
    mergedInto?: ObjectId; // set when a quick account is upgraded to Google
}
...
// Moves a quick account's pass and orders into a Google account (runs once per quick account)
export async function mergeGuestIntoGoogle(guestToken: string, googleAccountId: ObjectId) {
    const col = await accountsCollection();
    const now = Date.now();

    // Claim the quick account atomically, so two requests at once can't merge it twice
    const guest = await col.findOneAndUpdate(
        { type: 'guest', guestTokenHash: hashToken(guestToken), mergedInto: { $exists: false } },
        { $set: { mergedInto: googleAccountId, lastSeenAt: now } },
        { returnDocument: 'before' }
    );
    if (!guest || guest._id?.equals(googleAccountId)) return;

    // Move the pass: lifetime always wins, otherwise keep whichever day pass ends later
    if (guest.pass?.plan === 'lifetime') {
        await col.updateOne(
            { _id: googleAccountId },
            { $set: { pass: { plan: 'lifetime', expiresAt: null } } }
        );
    } else if (guest.pass?.plan === 'day' && (guest.pass.expiresAt ?? 0) > now) {
        const guestExpiry = guest.pass.expiresAt as number;
        await col.updateOne(
            {
                _id: googleAccountId,
                'pass.plan': { $ne: 'lifetime' },
                $or: [{ 'pass.expiresAt': null }, { 'pass.expiresAt': { $lt: guestExpiry } }],
            },
            { $set: { pass: { plan: 'day', expiresAt: guestExpiry } } }
        );
    }

    // Move their orders so payment status checks and refunds still find the right owner
    const client = await clientPromise!;
    await client
        .db('couplegames')
        .collection('orders')
        .updateMany({ accountId: guest._id }, { $set: { accountId: googleAccountId } });
}